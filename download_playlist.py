...
class TrackExportList():
    ...
    def export(self, remaining_items):
        ...
        for item in self.items:
            ...
            if not track_op.export_name in remaining_items.keys():
                track_op.download()
                track_op.export(self.directory)
...
class TracksFromYearTrackList(TrackExportList):
...
class AlbumTrackList(TrackExportList):
...
class ArtistTrackList(TrackExportList):
...
class LatestTrackList(TrackExportList):
...
class NamedPlayList(TrackExportList):
...
for query in export_queries:
    ...
    for smart_playlist_type in TrackExportList.__subclasses__():
...
for download_tracklist in export_tracklists:
    remaining_items = {}
    
    download_tracklist.set_up_directory(export_base_directory)

    for (dirpath, dirnames, filenames) in os.walk(download_tracklist.directory):
        for file in filenames:
            file_path = os.path.join(download_tracklist.directory, file)
    ...
    download_tracklist.export(remaining_items)
        
    if DELETE_REMOVED_TRACKS:
        print ("**** Deleting removed tracks")
        assert len(download_tracklist.items) > len(remaining_items)
